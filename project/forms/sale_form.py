...
class SaleProductForm(forms.ModelForm):
    class Meta:
        model = SaleProduct
        fields = ['company_name', 'name', 'amount', 'price', 'total_price', 'pay_status', 'type']
        labels={'company_name': _("Company Name"), 'name': _("Name"), 'amount': _("Amount"), 'price': _("Price"), 'total_price': _("Total Price"), 'pay_status': _("Pay Status"), 'type': _("Type")}
        widgets = {
    ...
    def save(self, commit=True):
        ...
        if not instance.pk:
            instance.sale_no = generate_unique_sale_no("SANO", SaleProduct, 'sale_no')
        if commit:
            instance.save()
        return instance